...
GROUND-ZERO에서 다시시작하자. 
실무자 마인드셋
본 학습 내용을 복습. --> 빠르게 전과정을 2회 이상 반복하자. 
주변 동료에게 자신을 알리고 궁금한 점을 준비해서 부탁하자. --> 부탁은 최소한. (알려주면 감사한 거고 왜 안가르쳐줘하면 )
굉장히 많은 이론을 알아야하는 건 알려줄 수 있지만, 나와 친하지 않으면 안알려줄 수도 있어요. 

협업하는 동료를 돕는 Yes맨이 되자. 결국 백엔드, 프론트엔드, 운영체제, 네트워크를 다 알아야한다. 
DB는 한번 배우면 계속 그대로이다. 
백엔드는 문제 해결과정에서 부딪히는 이슈는 공식문서, 깃헙, 레퍼런스에 계속 업데이트. 
30분 이내 결과 확인 가능한 가벼운 코딩 계속 할 것. 
공식문서를 자주 보고, 깃헙 레퍼런스를 자주 보고, 책이랑 강의를 계속 보라. 
하루에 30분씩 계속 자바랑 SQL은 계속 30분씩 문제를 풀어줘야함. 
...
-- 오라클 에러 중 가장 문제가 되는 것은 900번 에러입니다. 원인이 매우 당야하기 때문에 
-- 가장 높은 확률로는 2개인데
-- DESC(Description) 예약어 등의 SQL * PLUS 전용 키워드는 DBeaver나 Oracle에선 사용이 불가능하다. 
-- ==> 이 경우에는 테이블 선택 후 F4를 클릭하거나 F7을 클릭하세요. 

-- 오라클 포트 문제인 경우도 있다. 이건 구글링할 것. 
